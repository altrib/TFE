...
b = cbind(plot_spectrum(RACMO_Cabauw,column = 'w10m',spans=c(11,31))$spectrum_data,group='RACMO')
c = cbind(plot_spectrum(data_Cabauw,column = 'F010',spans=c(11,31))$spectrum_data,group='Measurements')
df_plot = rbind(a,b,c)
...
b = cbind(plot_spectrum(RACMO_Cabauw,column = 'w10m')$spectrum_data,group='RACMO')
c = cbind(plot_spectrum(data_Cabauw_3h,column = 'F010')$spectrum_data,group='Measurements 3h average')
df_plot = rbind(a,b,c)
...
qqplot(data_KNW$F010, data_Cabauw$F010, main = "Q-Q Plot of KNW vs Measurements",
...
pp_plot_compare(data_KNW$F010, data_Cabauw$F010, labs=labs)
```
...
```{r}
# Q-Q plot to compare data1 and data2
qqplot(RACMO_Cabauw$w10m, data_Cabauw$F010, main = "Q-Q Plot of RACMO vs Measurements",
...
pp_plot_compare(RACMO_Cabauw$w10m, data_Cabauw$F010, labs=labs)
```
...
```{r}
# Q-Q plot to compare data1 and data2
qqplot(RACMO_Cabauw$w10m, data_KNW$F010, main = "Q-Q Plot of RACMO vs KNW",
...
pp_plot_compare(RACMO_Cabauw$w10m, data_KNW$F010, labs=labs)
```
...
```{r}
# Q-Q plot to compare data1 and data2
qqplot(BM_select(RACMO_Cabauw,height = 'w10m')$max, BM_select(data_Cabauw)$max, main = "Q-Q Plot of RACMO vs KNW",
       xlab = "Quantiles of RACMO", ylab = "Quantiles of measurements")
abline(0, 1, col = "red")  # Adds a reference line y=x
...
pp_plot_compare(BM_select(RACMO_Cabauw,height = 'w10m')$max,BM_select(data_Cabauw)$max,labs)
```
...
plot(ecdf(data_Cabauw$F010),ylim = c(0,1),
     xlab = 'wind speed (m/s)',
     ylab = 'probability',
     main = paste0('Empirical CDFs of each whole dataset'))
lines(ecdf(data_KNW$F010),col='blue')

bm = BM_select(RACMO_Cabauw,height = 'w10m')
lines(ecdf(RACMO_Cabauw$F010),col='red')
par(mar = c(1, 1, 1, 2.3))
plot.new()
# Add a legend to the plot
legend(
...
pot = PoTselect(data_Cabauw$F010,p = 0.005,sep=6*12)$pot
...
pot = PoTselect(RACMO_Cabauw$w10m,p = 0.005,sep=4)$pot
...
```{r}
fit_RACMO = BM_fit(BM_select(RACMO_Cabauw,height = 'w10m'),plot=F)
...
trueTail=BM_fit(BM_select(data_Cabauw),plot=F)
modelTail = BM_fit(BM_select(data_Cabauw), kwargs = kwargs,plot=F)
...
fit_RACMO <- bootstrap(RACMO_Cabauw,Nb = 100,column = 'w10m',method = 'BM')
bm = BM_select(data_Cabauw)
...
```{r, message=FALSE, warning=FALSE, results='hide'}
WSEst_model_to_measure_BM(data_Cabauw,RACMO_Cabauw,col_model = 'w10m')
```
...
```{r, message=FALSE, warning=FALSE, results='hide'}
WSEst_model_to_measure_BM(data_Cabauw,data_KNW,col_model = 'w10m')
```
...
for (i in names(all_data)){
    ...
    if (!is.null(all_data[[i]][[col]])){
        out[[i]] <- bootstrap(all_data[[i]],Nb=300,column = col,method='BM')
...
On test si un resampling des mesures à Cabauw change beaucoup le résultat de l'estimation.
On resample toutes les 3h pour pouvoir comparé avec RACMO.
```{r, message=FALSE, warning=FALSE, results='hide',eval=FALSE}
col = 'F010'
alpha = 0.05
labels = minutes(seq(0,170,10))
names(data_Cabauw_3h) = labels 
names(labels) = names(data_Cabauw_3h)
out = list()
lower_bounds = c()
upper_bounds =  c()

for (i in names(data_Cabauw_3h)){
    print(i)
    if (!is.null(data_Cabauw_3h[[i]][[col]])){
        out[[i]] <- bootstrap(data_Cabauw_3h[[i]],Nb=300,column = col,method='BM')
        lower_bounds[i] = out[[i]]$original[1] - qnorm(1 - alpha / 2) * sd(out[[i]]$df$tail)
        upper_bounds[i] = out[[i]]$original[1] + qnorm(1 - alpha / 2) * sd(out[[i]]$df$tail)
    }
    else{
...
ggplot(plot_data, aes(x = x, y = tail_index, group = group)) +
  ...
  theme_minimal()
```
On ne peut pas trop conclure la dessus.



# Différentes estimation du vent pour les 3 méthodes étudiées

```{r, message=FALSE, warning=FALSE, results='hide',eval=FALSE}
a = WSEst_BM(bootstrap(data_Cabauw,Nb = 100,column = 'F010',method = 'BM'),bm = BM_select(data_Cabauw), trueTail = BM_fit(BM_select(data_Cabauw),plot = F))$pred
b = WSEst_BM(bootstrap(RACMO_Cabauw,Nb = 100,column = 'w10m',method = 'BM'),bm = BM_select(RACMO_Cabauw,height = 'w10m'), trueTail = BM_fit(BM_select(RACMO_Cabauw,height = 'w10m'),plot = F))$pred
c = WSEst_GP(data_Cabauw,timestep = 1/6,threshold = 0.1)$df
d = WSEst_GP(RACMO_Cabauw,col = 'w10m',timestep = 3,threshold = 0.1)$df
block_length=  "1y"
e = WSEst_GW(data_Cabauw,timestep = 1/6, threshold = 0.1)$df
f = WSEst_GW(RACMO_Cabauw,timestep = 3, threshold = 0.1,col = "w10m")$df


gg = ggplot(a, aes(x = return)) +
      geom_ribbon(aes(ymin = lb, ymax = ub, fill = 'Meas BM'), alpha = 0.1) +
      geom_line(aes(x=return,y=original,color = 'Meas BM')) +
  
      geom_ribbon(data = b,aes(ymin = lb, ymax = ub, fill = 'RACMO BM'), alpha = 0.2) +
...
ggplot(a, aes(x = return)) +
      ...
      geom_ribbon(data = c,aes(ymin = lb, ymax = ub, fill = 'Meas GP'), alpha = 0.1) +
      geom_line(data = c,aes(x=return,y=original,color = 'Meas GP')) +
  
      geom_ribbon(data = e,aes(ymin = lb, ymax = ub, fill = 'Meas GW'), alpha = 0.1) +
      geom_line(data = e,aes(x=return,y=original,color = 'Meas GW')) +
      xlim(c(50,10000))+
      scale_x_log10(labels= scales::comma) +
...
ggplot(b, aes(x = return)) +
      geom_ribbon(aes(ymin = lb, ymax = ub, fill = 'RACMO BM'), alpha = 0.2) +
      geom_line(data = b,aes(x=return,y=original,color = 'RACMO BM')) +
  
      geom_ribbon(data = d,aes(ymin = lb, ymax = ub, fill = 'RACMO GP'), alpha = 0.2) +
      geom_line(data = d,aes(x=return,y=original,color = 'RACMO GP')) +
  
  
      geom_ribbon(data = f,aes(ymin = lb, ymax = ub, fill = 'RACMO GW'), alpha = 0.1) +
      geom_line(data = f,aes(x=return,y=original,color = 'RACMO GW')) +
      xlim(c(50,10000))+
      scale_x_log10(labels= scales::comma) +
...
test <- function(data,separation,column = "F010"){
bm = bootstrap(data,Nb=100,column = column,method='BM')
...
test(RACMO_Cabauw,4,column = 'w10m')
```
